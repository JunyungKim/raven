...
class GeneticAlgorithm(RavenSampled):
  ...
  def getInputSpecification(cls):
    ...
    specs.description = r"""The \xmlNode{GeneticAlgorithm} is a heuristic optimization technique inspired by the principles 
                            ...
                            potential solutions represented as strings of binary digits (genes). These solutions undergo iterative refinement 
                            through processes such as mutation, crossover, and survivor selection. Mutation involves randomly altering certain genes within 
  ...
  def handleInput(self, paramInput):
    ...
    else: self._numOfConst = 0

    # Sanity Check
    if fitnessNode.findFirst('a') is not None and len(self._objectiveVar) != len(fitnessNode.findFirst('a').value): # Error Case 1: The number of objective coefficients in <a> is NOT identical with the number of objectives. 
      self.raiseAnError(IOError, f'The number of objective coefficients in <a> must match the number of objectives. ')      
    if fitnessNode.findFirst('b') is not None and len(fitnessNode.findFirst('b').value) != len(self._objectiveVar)*self._numOfConst : # Error Case 2: The number of penaltyCoeff. in <b> is NOT identical with the number of objective in <objective> multiplied by the number of constraints
      self.raiseAnError(IOError, f'The number of penalty coefficient provided in <b> must match the number of objectives multiplied by the numebr of constraints. ')  

    self._objCoeff = fitnessNode.findFirst('a').value if fitnessNode.findFirst('a') is not None else [1] * len(self._objectiveVar) 
    if self._numOfConst != 0 and fitnessNode.findFirst('b') is not None: # Error Case 3: constraints exist and constraint coefficients given.    
      self._penaltyCoeff = fitnessNode.findFirst('b').value
    elif self._numOfConst != 0 and fitnessNode.findFirst('b') is None:   # Error Case 4: constraints exist and No constraint coefficients given (if penaltyCoeff is not provided, then assume they are all 1).   
      self._penaltyCoeff = [1] * self._numOfConst * len(self._objectiveVar) 
    else: # Error Case 5: No constraints exist. 
      self._penaltyCoeff = [0] * self._numOfConst * len(self._objectiveVar) 
